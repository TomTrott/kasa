...
// Version statique : simule un upload réseau sans appeler de vraie API.
// On réutilise simplement une URL locale (object URL) comme "URL distante" simulée,
// après un petit délai pour imiter une vraie requête (utile pour Storybook / démo).
...
export interface AddPropertiesStoryProps {
  /** Appelé quand l'utilisateur clique sur "Retour". Sans callback, on retombe sur window.history.back(). */
  onBack?: () => void;
  /** Appelé après une soumission "réussie" simulée, avec un id factice. */
  onSubmitSuccess?: (id: string) => void;
}

/**
 * Version statique de la page "Ajouter une propriété", sans dépendance à Next.js (useRouter)
 * ni à un client API réel. Toutes les interactions (équipements, tags, images, lightbox,
 * soumission) fonctionnent côté client, mais l'upload et la création de propriété sont simulés.
 * Pensée pour être rendue isolément, par exemple dans Storybook.
 */
export default function AddPropertiesStory({
  onBack,
  onSubmitSuccess,
}: AddPropertiesStoryProps) {
  // Champs texte
  ...
  // Retour : callback fourni par le parent (Storybook, page hôte...) ou fallback navigateur
  ...
  // Soumission du formulaire — version statique : aucun appel réseau réel
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      // Les "uploads" simulés partent en parallèle, comme dans la version connectée
      const [coverUrl, pictureUrls, hostPictureUrl] = await Promise.all([
        coverFile ? mockUploadImage(coverFile) : Promise.resolve(undefined),
        Promise.all(pictureFiles.map((file) => mockUploadImage(file))),
        hostPictureFile ? mockUploadImage(hostPictureFile) : Promise.resolve(undefined),
      ]);
      ...
      // Pas de vraie création de propriété : on simule juste un petit délai réseau
      await new Promise((resolve) => setTimeout(resolve, 500));
      // eslint-disable-next-line no-console
      console.log("[addpropertiesstory] Payload simulé :", payload);
      onSubmitSuccess?.("static-demo-id");
    } catch {
